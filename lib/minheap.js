...
class MinHeap {
  ...
  getChildIndicesAndComparisons(index) {
    ...
    return { leftChildIndex, rightChildIndex, lesserChildIndex, sortsAboveLeft, sortsAboveRight };
  }
  ...
  heapDown(index) {
    ...
    const {
      ...
      sortsAboveLeft,
      sortsAboveRight
    } = this.getChildIndicesAndComparisons(index);

    if ((sortsAboveLeft && sortsAboveRight)) {
      return;
    }

    if (this.outsideHeap(leftChildIndex)) {
      // There are no children to swap with.
      return;
    }
    ...
    const swapAndHeapDown = childIndex => {
      this.swap(index, childIndex);
      this.heapDown(childIndex);
    }

    if (sortsAboveLeft && !sortsAboveRight) {
      swapAndHeapDown(rightChildIndex);
    } else if (sortsAboveRight && !sortsAboveLeft) {
      swapAndHeapDown(leftChildIndex);
    } else {
      swapAndHeapDown(lesserChildIndex);
    }
  }
